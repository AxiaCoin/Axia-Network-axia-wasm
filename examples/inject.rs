...
use axia_wasm::elements;
use axia_wasm::builder;

pub fn inject_nop(instructions: &mut elements::Instructions) {
	use axia_wasm::elements::Instruction::*;
...
fn main() {
	...
	let mut module = axia_wasm::deserialize_file(&args[1]).unwrap();
	...
	axia_wasm::serialize_to_file(&args[2], build.build()).unwrap();
}
